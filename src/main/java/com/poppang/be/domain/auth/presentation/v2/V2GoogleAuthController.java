...
@RequestMapping(value = "/api/v2/auth/google", produces = MediaType.APPLICATION_JSON_VALUE)
public class V2GoogleAuthController {

  private final V2GoogleAuthService authService;
  ...
  public ResponseEntity<ApiResponse<V2SocialAuthResponseDto>> mobileLogin(
      @RequestBody(required = false) V2GoogleMobileLoginRequestDto request) {
    return noStore(authService.mobileLogin(request == null ? null : request.idToken()));
  }

...
class V2KakaoSignupWriter {
  ...
  private ValidatedSignupInput validate(V2SignupRequestDto request) {
    ...
    List<String> keywords =
        Optional.ofNullable(request.alertKeywordList()).orElseGet(List::of).stream()
            .map(keyword -> keyword == null ? null : keyword.trim())
            .filter(Objects::nonNull)
            .toList();
    if (keywords.size()
            != Optional.ofNullable(request.alertKeywordList()).orElseGet(List::of).size()
        || keywords.stream().anyMatch(String::isBlank)) {
      throw new BaseException(ErrorCode.INVALID_SIGNUP_REQUEST);
    }

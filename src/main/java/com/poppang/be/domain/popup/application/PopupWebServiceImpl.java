...
public class PopupWebServiceImpl implements PopupWebService {
    ...
    public List<PopupWebRandomResponseDto> getRandomPopupList() {
        ...
        List<PopupWebRandomResponseDto> randomPopupList = popupRepository.findRandomActiveWithThumbnail(RANDOM_LIMIT)
                .stream()
                .map(r -> PopupWebRandomResponseDto.builder()
    ...
    public List<PopupWebFavoriteResponseDto> getFavoritePopupList() {
        ...
        List<PopupWebFavoriteResponseDto> favorietPopupList = popupRepository.findTopViewedActiveWithThumbnail(FAVORITE_LIMIT)
                .stream()
                .map(r -> PopupWebFavoriteResponseDto.builder()
                        .popupUuid(r.getPopupUuid())
                        .name(r.getPopupName())
                        .thumbnailUrl(r.getThumbnailUrl())
                        .region(r.getRegion())
    ...
    public List<PopupWebUpcomingResponseDto> getUpcomingPopupList() {
        ...
        List<PopupWebUpcomingResponseDto> upcomingPopupList = popupRepository.findUpcomingActiveWithThumbnail(startDate, endDate, UPCOMING_LIMIT)
                .stream()
                .map(r -> PopupWebUpcomingResponseDto.builder()
                        .popupUuid(r.getPopupUuid())
                        .name(r.getPopupName())
                        .thumbnailUrl(r.getThumbnailUrl())
                        .region(r.getRegion())
